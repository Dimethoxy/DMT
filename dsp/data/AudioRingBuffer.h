...
public:
  ...
  */
  ...
  //============================================================================
  /**
      Writes the newest audio data from the bufferToWrite into the buffer.

      @param bufferToWrite The buffer containing the audio data to write.
      As this will only write the newest data, the bufferToWrite can be
      smaller than the RingBuffer.
  */
  const OperationResult writeNewest(
    const AudioBuffer<SampleType>& bufferToWrite) noexcept
  {
    // If bufferToWrite is larger than the RingBuffer we need to return early
    if (bufferToWrite.getNumChannels() > numChannels) {
    ...
    // Iterate the samples first to improve thread safety, this time we need
    // to iterate backwards to get the newest data

    for (int sample = bufferToWrite.getNumSamples() - 1; sample >= 0;
         --sample) {
      const int ringPosition = (startingPosition + sample) % bufferSize;
      for (int channel = 0; channel < numChannels; ++channel) {
        buffer->copyFrom(
...
private:
  ...
  AudioBuffer<SampleType> buffer; // Buffer to store audio data
