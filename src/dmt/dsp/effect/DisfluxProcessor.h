 ...
 *
 * Description:
 * Neutrino Processor class for processing audio buffers with a series of
 * all-pass filters.
 *
 * Authors:
...
public:
  //==============================================================================
  /**
   * @brief Constructs a NeutrinoProcessor with the given parameters.
   *
  ...
  inline void processBlock(AudioBuffer& _buffer) noexcept
  {
    ...
    // Load parameters
    const int newAmount = apvts.getRawParameterValue("NeutrinoAmount")->load();
    const int newSpread = apvts.getRawParameterValue("NeutrinoSpread")->load();
    const auto newFrequency =
      apvts.getRawParameterValue("NeutrinoFrequency")->load();
    const auto newPinch = apvts.getRawParameterValue("NeutrinoPinch")->load();
    const auto mix = apvts.getRawParameterValue("NeutrinoMix")->load();
