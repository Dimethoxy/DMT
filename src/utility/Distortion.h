...
struct Distortion {
  ...
  static inline void distortSample(float &value, dmt::Distortion::Type type,
                                   float drive) {
    switch (type) {
    case Type::Hardclip: {
      value = value * drive;
  ...
  static inline void processBuffer(juce::AudioBuffer<float> &buffer,
                                   dmt::Distortion::Type type, float symmetry,
                                   float girth, float drive) {
    std::vector<float> girthSeeds;
    if (girth < 0)
    ...
    for (int channel = 0; channel < buffer.getNumChannels(); channel++) {
      ...
      for (int sample = 0; sample < buffer.getNumSamples(); sample++) {
        // Girth
        if (girth < 0)
          dmt::Distortion::girthSample(channelData[sample], std::abs(girth),
                                       girthSeeds[sample]);
        else
